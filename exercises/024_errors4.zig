...
fn makeJustRight(n: u32) MyNumberError!u32 {
    return fixTooBig(n) catch |result| {
        return result;
    };
}

fn fixTooBig(n: u32) MyNumberError!u32 {
    return fixTooSmall(n) catch |result| {
        if (result == MyNumberError.TooBig) {
            return 20;
        }
...
fn fixTooSmall(n: u32) MyNumberError!u32 {
    ...
    return detectProblems(n) catch |result| {
        if (result == MyNumberError.TooSmall) {
            return 10;
        }
        return result;
    };
